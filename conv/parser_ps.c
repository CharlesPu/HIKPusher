...
@Others: 
*************************************************/
int PS_ProcessHeadPacket(char** Pack, int *length)  
{  
...
@Others: 
*************************************************/  
int PS_ProcessMapPacket(char** Pack, int* length, char **PayloadData, int *PayloadDataLen)  
{ 
...
*************************************************/ 
int PS_ProcessPESPacket(char** Pack, int *length, char **PayloadData, int *PayloadDataLen)  
{  
    ...
    *length = (*length) - data_length.length - sizeof(PS_EACH_PACKET_START) - sizeof(PS_2BYTES_LENGTH);    
    *Pack = (*Pack) + sizeof(PS_EACH_PACKET_START) + sizeof(PS_2BYTES_LENGTH) + data_length.length;  
...
{  
    ...
    {           
        ...
        {  
            if(PS_ProcessHeadPacket(&next_posi, &left_len) == 0)
                break;  
        }else 
        if(next_posi   
        && next_posi[0] == '\x00'   
        ...
        {  
            //接着就是流包，说明是非i帧  
            if(PS_ProcessPESPacket(&next_posi, &left_len, &PayloadData, &PayloadDataLen))  
            {  
                if(PayloadDataLen)  
                {   
        ...
        {  
            if(PS_ProcessMapPacket(&next_posi, &left_len, &PayloadData, &PayloadDataLen) == 0)  
                break;  
        }  
        else   //如果啥都不是
        {
